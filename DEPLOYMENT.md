# Vediex VPS Deployment Guide (Fresh OS Installation)
...
PORT=5001
NODE_ENV=production
MONGODB_URI=mongodb://localhost:27017/Vediex
...
# Configure nginx
sudo nano /etc/nginx/sites-available/Vediex
```
...
Enable the site:
```bash
sudo ln -s /etc/nginx/sites-available/Vediex /etc/nginx/sites-enabled/
sudo nginx -t
...
pm2 stop Vediex-backend   # Stop backend
pm2 delete Vediex-backend # Remove from PM2
```
...
### View logs
```bash
pm2 logs Vediex-backend --lines 100
```
